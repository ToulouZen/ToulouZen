...
import { View, Text, TextInput, TouchableOpacity, Image, ActivityIndicator, Touchable } from 'react-native'
...
const DriverConfirmScreen: React.FC<Props> = ({ navigation, route }) => {
    ...
    React.useEffect(() => {
        if (firestore.actualPath != undefined) {
            setRegion({ latitude: firestore.actualPath.arrivalDestination.latitude, longitude: firestore.actualPath.arrivalDestination.longitude, latitudeDelta: 0.0922, longitudeDelta: 0.0421 })
        }
            ...
            <View style={[styles.container, { backgroundColor: COLORS.white, flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }]}>
                <Overlay isVisible={modalVisible} fullScreen animationType="slide">
                    <View style={{ flex: 1, backgroundColor: "transparent", justifyContent: 'center', alignItems: 'center', flexDirection: 'column' }}>
                        <Map destination={firestore.actualPath?.arrivalDestination} region={region} passengerPosition={firestore.actualPath?.departureDestination} />
                        ...
                        <>
                            <View style={{ width: '100%', justifyContent: 'space-between', alignItems: 'center', flexDirection: 'row', marginTop: 20 }}>
                                <Text style={[styles.logTexts, { color: COLORS.peach, textAlign: 'center' }]}>
                                    En attente de la validation de {firestore.actualPath?.userFirstname} {firestore.actualPath?.userLastname}
                                </Text>

                            </View>
                            <ActivityIndicator color="#52ad8d" style={{ marginTop: 50 }} size="large" />
                        </>
                    }
                    {
